...
(AWS ECS Deployment Version - fetches secrets from AWS Secrets Manager)
...
- Cache hits reduce cost by 10x ($3.75/M -> $0.375/M for input tokens)
...
- On ECS, secrets are fetched from AWS Secrets Manager using boto3
- Secret name is configured via AWS_SECRET_NAME env var (default: "agentic-search/anthropic")
- AWS region is configured via AWS_REGION env var (default: "us-east-1")
- Expected secret JSON format: {"api_key": "...", "base_url": "..."}
- Secrets are cached in-memory to avoid repeated API calls
- Falls back to environment variables (ANTHROPIC_API_KEY) for local development
"""
import json
import logging
import os
...
_secrets_cache_ttl: Dict[str, float] = {}
...
def get_secret_from_aws(secret_name: str, region_name: str) -> dict:
    ...
    On ECS, the task role provides credentials automatically (no explicit
    access key / secret key needed).
    ...
    if secret_name in _secrets_cache:
        if now - _secrets_cache_ttl.get(secret_name, 0) < SECRETS_CACHE_DURATION:
    ...
    try:
        ...
        _secrets_cache_ttl[secret_name] = now

        logger.info(f"Successfully fetched secret '{secret_name}' from AWS Secrets Manager")
        return secret_dict
...
def _resolve_credentials(
    ...
    """
    Resolve api_key and base_url from AWS Secrets Manager, falling back
    to environment variables for local development.

    Resolution order:
      1. Explicit constructor arguments (api_key / base_url)
      2. AWS Secrets Manager (if AWS_SECRET_NAME is set or running on ECS)
      3. Environment variables (ANTHROPIC_API_KEY)
      4. Default base_url: https://api.anthropic.com
    ...
    # Attempt AWS Secrets Manager if we don't already have both values
    if not resolved_api_key or not resolved_base_url:
        secret_name = os.getenv("AWS_SECRET_NAME", "agentic-search/anthropic")
        region_name = os.getenv("AWS_REGION", "us-east-1")

        # Detect if we're likely running on ECS (task metadata endpoint exists)
        # or if the user explicitly set AWS_SECRET_NAME
        on_ecs = os.getenv("ECS_CONTAINER_METADATA_URI_V4") is not None
        explicit_secret = os.getenv("AWS_SECRET_NAME") is not None

        if on_ecs or explicit_secret:
            try:
                secret = get_secret_from_aws(secret_name, region_name)
                if not resolved_api_key:
                    resolved_api_key = secret.get("api_key")
                if not resolved_base_url:
                    resolved_base_url = secret.get("base_url")
                logger.info("Credentials resolved from AWS Secrets Manager")
            except Exception as e:
                logger.warning(
                    f"Could not fetch from AWS Secrets Manager, "
                    f"falling back to env vars: {e}"
                )
...
class ClaudeClient:
    ...
    def __init__(
        ...
        # Resolve credentials from AWS Secrets Manager or env vars
        ...
        if not self.api_key:
            raise ValueError(
                ...
                "  1. AWS Secrets Manager (set AWS_SECRET_NAME env var), or\n"
                "  2. ANTHROPIC_API_KEY environment variable, or\n"
                "  3. api_key constructor parameter"
            )

        logger.info(f"Initialized Claude client: {self.model} (base_url={self.base_url})")
    ...
    async def generate_response(
        ...
        Args:
            ...
            cacheable_prefix: Optional static content to cache (e.g., tool list, instructions)
                            This content will be cached for 5 minutes, reducing cost by 10x
        ...
        try:
            ...
            if "usage" in result:
                ...
                if cache_read > 0:
                    logger.info(f"Cache HIT: {cache_read} tokens loaded from cache (10x cheaper!)")
...
# On ECS: fetches api_key + base_url from AWS Secrets Manager
